# Btrfs Subvolume Snapshot Utility (version: 1.1.0)
...
* revert:   revert makes snapshot <ISO_8601_date>.last and replace contents of
...
DESTINATION_PATH is a local path such as /srv/backup, then "rsync -aHxSv --delete"
is used to save the CPU load.  If DESTINATION_PATH is a remote path such as
"[USER@]HOST:DESTINATION_PATH", then "rsync -aHxSzv --delete" is used to save the
network load.
